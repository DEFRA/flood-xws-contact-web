...
const { ViewModel, schema, customErrors } = require('../models/mob')
...
module.exports = [
  {
    ...
    handler: async (request, h) => {
      ...
      return h.view('mob', new ViewModel({ consent, mobile }))
    }
  },
  {
    method: 'POST',
    ...
    handler: async (request, h) => {
      ...
      if (!consent) {
        ...
        try {
          ...
          return h.view('mob', new ViewModel(payload, new Errors(customErrors.parseError)))
        }
        ...
        if (!isCorrectType) {
          return h.view('mob', new ViewModel(payload, new Errors(customErrors.incorrectFormat)))
        }
    ...
    options: {
      validate: {
        ...
        failAction: (request, h, err) => {
          return h.view('mob', new ViewModel(request.payload, Errors.fromJoi(err))).takeover()
        }
      }
    }
  }
